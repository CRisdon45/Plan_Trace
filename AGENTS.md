# Working on Plan Trace

Build for Cody designing complete pool/landscape projects with a client, not for generic tracing or SaaS feature parity. Optimize the application for his working rhythm and the repository for inspectable, reproducible development.

## Re-enter the project

Start with README.md, docs/CURRENT_STATE.md and docs/PRODUCT_VISION.md. Verify the live branch/head and local changes. The default branch may carry current context without current app code. Consult docs/DECISIONS.md and only the contracts/evidence needed for the task. Historical reports explain past evidence, not today's instructions.

The latest explicit owner request controls scope. CURRENT_STATE records the checkpoint; the product/decision documents govern direction, and source/tests establish implementation facts. Flag contradictions instead of silently inventing approval or treating aspirations as shipped features.
