...
class DesignDocumentOutputTest {
    ...
    @Test fun `appearance changes only transient object driven presentation`() {
        ...
        assertEquals(listOf(1.6f,1.6f,2.0f,2.5f),technical.map { it.strokeWidth })
        assertEquals(technical.map { it.strokeWidth },graphic.map { it.strokeWidth })
        assertEquals(listOf(1.4f,1.25f,2.35f,2.5f),northstar.map { it.strokeWidth })
