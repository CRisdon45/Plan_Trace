...
/**
 * Android-native Filament projection of the authoritative TraceProject.
 *
 * The renderer owns GPU state only. Plan data remains authoritative in TraceProject and the
 * semantic 3D adapter. Keeping this boundary narrow lets Perspective evolve without coupling the
 * tablet drawing core to rendering implementation details.
 */
class FilamentPlanSurface(context: Context) : TextureView(context) {
    ...
    private fun rebuildMeshes(faces: List<Face3D>) {
        ...
        // Plan coordinates use +Y downward. Convert them into a conventional right-handed
        // Filament scene while keeping the source semantic model unchanged.
    ...
    private fun createMesh(
        ...
        RenderableManager.Builder(1)
            ...
            // Filament's Builder.culling flag is CPU frustum culling, not material face culling.
            // Keep it off until scene bounds are mature; the single-sided material still performs
