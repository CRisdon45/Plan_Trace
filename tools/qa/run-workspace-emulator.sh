...
run_case snapToProtectedGeometry com.example.GeometrySnapDeviceTest
device shell am force-stop "$package"
device exec-out run-as "$package" cat files/project-design/workspace.json > emulator-evidence/geometry-snap-before-restart.json
run_case snapResultReopensWithoutTemporaryConstraints com.example.GeometrySnapDeviceTest
device shell am force-stop "$package"
